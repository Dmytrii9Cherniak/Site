...
function InfoPage() {
    ...
    const validateForm = () => {
        ...
        if (!isChecked) {
            errors.checkbox = 'Ви повинні прийняти умови про обробку персональних данних'
        }
    ...
    return (<div className="info_page">
        ...
        <div className={`form_message ${isModalWindowOpened ? 'isModalWindowOpened' : ''}`}>
            ...
            {!errors.name && errors.email && <p className="error">{errors.email}</p>}
            {!errors.name && !errors.email && errors.number && <p className="error">{errors.number}</p>}
            {!errors.name && !errors.email && !errors.number && errors.checkbox && <p className="error">{errors.checkbox}</p>}
            <div>
